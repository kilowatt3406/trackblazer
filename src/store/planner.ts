...
export const usePlannerStore = create<PlannerState>((set, get) => ({
  ...
  toggleRace: (turn: number, raceId: number) => {
    set((state) => {
      ...
      if (raceIndex >= 0) {
        turnData.selectedRaceIds = turnData.selectedRaceIds.filter(
          (id) => id !== raceId,
        );
      } else {
  ...
  getSelectedRaces: () => {
    ...
    for (const turn of state.turns) {
      for (const raceId of turn.selectedRaceIds) {
        const turnRace = turn.availableRaces.find(
          (tr) => tr.race.id === raceId,
        );
        if (turnRace) {
          result.push({
            turn: turn.turn,
            race: turnRace.race,
            uraId: turnRace.uraId,
            year: turnRace.year,
          });
        }
      }
  ...
  savePlan: (name: string) => {
    ...
    const plan: Plan = {
      ...
      selectedRaces: state.turns.reduce(
        (acc, t) => {
