...
export function Optimizer() {
  const { selectedEpithetIds, turns, clearAllRaces, toggleRace } =
    usePlannerStore();
    ...
    for (const epithet of targetEpithets) {
      ...
      if (desc.includes("g1")) {
        for (const turn of turns) {
          const g1Races = turn.availableRaces.filter(
            (tr) => tr.race.grade === 100,
          );
          if (g1Races.length > 0) {
        ...
        for (const turn of turns) {
          const g2Races = turn.availableRaces.filter(
            (tr) => tr.race.grade === 200,
          );
          if (g2Races.length > 0) {
            const alreadySelected = newSelections.some(
              (s) => s.turn === turn.turn,
        ...
        for (const turn of turns) {
          const g3Races = turn.availableRaces.filter(
            (tr) => tr.race.grade === 300,
          );
          if (g3Races.length > 0) {
            const alreadySelected = newSelections.some(
              (s) => s.turn === turn.turn,
        ...
        for (const turn of turns) {
          const dirtRaces = turn.availableRaces.filter(
            (tr) => tr.race.terrain === 2,
          );
